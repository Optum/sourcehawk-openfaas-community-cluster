...
# shellcheck disable=SC2162 disable=SC2039 FIXME
#if read -t 0; then
...
if 2>"$ERROR_OUTPUT_FILE" ./bin/sourcehawk scan --verbosity MEDIUM --output-format "$OUTPUT_FORMAT" --config-file "$CONFIG_FILE" "$SOURCE_CODE_ROOT_DIRECTORY"; then
   exit 0
else
  SCAN_EXIT_CODE=$?
